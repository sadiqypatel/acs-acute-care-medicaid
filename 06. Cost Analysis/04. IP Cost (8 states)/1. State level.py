...
percent = percent.select("state_name","member_month","ip_percent")
percent = percent.withColumnRenamed("state_name", "state")
combined = ip_spend.join(percent, how="left", on="state")
combined.show()
...
select sum(avoid_ip_cost) as avoid_ip_cost, sum(member_month) as member_month
FROM combined ; 
''')

spend_total = spend_total.withColumn("pmpy", (col("avoid_ip_cost") / col("member_month")) * 12)
spend_total.show()