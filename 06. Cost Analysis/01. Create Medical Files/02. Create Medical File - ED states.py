...
inpatient_filtered = inpatient.filter(col("state").isin(["AR","CO","DE","FL","IL","IN","MI","MS","NM","SC","AK","ME","MT","SD","WY"]))
print(inpatient_filtered.count())

# COMMAND ----------

# Use the filter transformation
outpatient_filtered = outpatient.filter(col("state").isin(["AR","CO","DE","FL","IL","IN","MI","MS","NM","SC","AK","ME","MT","SD","WY"]))
print(outpatient_filtered.count())

# COMMAND ----------

inpatient_filtered.write.saveAsTable("dua_058828_spa240.paper2_ed_cost_ip_file", mode='overwrite')
outpatient_filtered.write.saveAsTable("dua_058828_spa240.paper2_ed_cost_other_file", mode='overwrite')
