...
select fips_code, sum(total_amt1) as total_amt1, sum(total_amt2) as ffs_amount_ip, sum(total_amt3) as mc_amount_ip
FROM spend_fips 
GROUP BY fips_code; 
''')
...
spend_county.registerTempTable("spend_fips")
spend_state = spark.sql('''
select fips_code, sum(ffs_amount) as ffs_amount_other, sum(mc_amount) as mc_amount_other
FROM spend_fips 
GROUP BY fips_code; 
''')
...
spend_county.registerTempTable("spend_fips")
spend_state = spark.sql('''
select fips_code, state, sum(ffs_amount) as ffs_amount_ed, sum(mc_amount) as mc_amount_ed
FROM spend_fips 
...
combined = ip_spend.join(ed_spend, on="fips_code", how="left")
combined = combined.join(other_spend, on="fips_code", how="left")
combined = combined.select("fips_code",'state',"inpatient_cost","total_cost_ed","total_cost_other")
#combined.show()

# COMMAND ----------

percent = spark.table("dua_058828_spa240.paper2_acute_care_percent_county")
percent = percent.select("fips_code","member_month","ip_percent","ed_percent","acute_percent")
...
combined = combined.withColumn("pmpy_ed", (col("avoid_ed_cost") / col("member_month")) * 12)
combined = combined.withColumn("pmpy_ip", (col("avoid_ip_cost") / col("member_month")) * 12)
#combined.show()

# COMMAND ----------

combined = combined.select("fips_code","state","member_month","pmpy_ed","pmpy_ip")
combined.show()
...
deciles = combined.approxQuantile("pmpy_ed", quantile_values, 0.01)
...
# Calculate the deciles
quantile_values = [0.0,  0.25, 0.5, 0.75, 1.00]
deciles = combined.approxQuantile("pmpy_ip", quantile_values, 0.01)
...
grouped_df = combined.groupBy("state").agg(
    expr("percentile_approx(pmpy_ed, 0.50)").alias("pmpy_ed_med"),
  expr("percentile_approx(pmpy_ip, 0.50)").alias("pmpy_ip_med")
)
combined = combined.join(grouped_df, on="state", how="left")
combined.show()
...
combined = combined.withColumn(
    "saving_ed",
    when(col("pmpy_ed") > col("pmpy_ed_med"), 
         (col("pmpy_ed") - col("pmpy_ed_med")) * (1/12) * col("member_month"))
    .otherwise(0)
)

combined = combined.withColumn(
    "saving_ip",
    when(col("pmpy_ip") > col("pmpy_ip_med"), 
         (col("pmpy_ip") - col("pmpy_ip_med")) * (1/12) * col("member_month"))
    .otherwise(0)
)
...
#aggregate to total 
combined.registerTempTable("spend")
combined_state = spark.sql('''
select state, sum(saving_ed) as saving_ed, sum(saving_ip) as saving_ip, sum(member_month) as months
FROM spend 
GROUP BY state; 
''')

combined_state = combined_state.withColumn("pmpy_ip_saving", (col("saving_ip") / col("months")) * 12)
combined_state = combined_state.withColumn("pmpy_ed_saving", (col("saving_ed") / col("months")) * 12)
combined_state = combined_state.withColumn("saving_total", (col("saving_ed") + col("saving_ip")))
combined_state = combined_state.withColumn("pmpy_total_saving", ((col("saving_ed") + col("saving_ip")) / col("months")) * 12)
...
#aggregate to total 
combined.registerTempTable("spend")
combined_total = spark.sql('''
select sum(saving_ed) as saving_ed, sum(saving_ip) as saving_ip, sum(member_month) as months
FROM spend; 
''')

combined_total = combined_total.withColumn("pmpy_ip_saving", (col("saving_ip") / col("months")) * 12)
combined_total = combined_total.withColumn("pmpy_ed_saving", (col("saving_ed") / col("months")) * 12)
combined_total = combined_total.withColumn("saving_total", (col("saving_ed") + col("saving_ip")))
combined_total = combined_total.withColumn("pmpy_total_saving", ((col("saving_ed") + col("saving_ip")) / col("months")) * 12)
...
# Calculate the deciles
quantile_values = [0.0,  0.25, 0.5, 0.75, 1.00]
deciles = combined_state.approxQuantile("pmpy_ed_saving", quantile_values, 0.01)
...
# Calculate the deciles
quantile_values = [0.0,  0.25, 0.5, 0.75, 1.00]
deciles = combined_state.approxQuantile("pmpy_ip_saving", quantile_values, 0.01)
