...
select state, fips_code, sum(ffs_amount) as ffs_amount_ed, sum(mc_amount) as mc_amount_ed
FROM spend_fips 
...
percent = percent.select("fips_code","member_month","ed_percent")
combined = ed_spend.join(percent, how="left", on="fips_code")
combined.show()
...
df_select = combined.select("fips_code","state","avoid_ed_cost","pmpy")
df_select = df_select.withColumnRenamed("pmpy", "ed_pmpy")
df_select.show()

# COMMAND ----------

df_select.write.format('csv').option('header', 'true').mode('overwrite').save("dbfs:/mnt/dua/dua_058828/SPA240/files/paper2_ed_spend.csv")

# COMMAND ----------

dbutils.fs.cp(f"dbfs:/mnt/dua/dua_058828/SPA240/files/paper2_ed_spend.csv", "s3://apcws301-transfer/dua/dua_058828/toVDI/paper2_ed_spend.csv", recurse=True)
