...
spend_county.registerTempTable("spend_county")
...
patient_list = patient_list.filter(col("state").isin(["AR","CO","DE","FL","IL","IN","MI","MS","NM","SC","AK","ME","MT","SD","WY"]))
print(patient_list.count())
patient_list.show()
...
combined = combined.select("beneID", "state", "fips_code", "total_cost_ed")
