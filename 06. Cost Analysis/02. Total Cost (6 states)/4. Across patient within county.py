...
select state, beneID, CLM_ID, sum(total_amt1) as total_amt1, mean(total_amt2) as total_amt2, mean(total_amt3) as total_amt3
FROM spend 
GROUP BY state, beneID, CLM_ID; 
''')
...
df_ip.registerTempTable("spend")
spend_county = spark.sql('''
select state, beneID, CLM_ID, sum(ffs_amount) as ffs_amount, sum(mc_amount) as mc_amount
FROM spend 
GROUP BY state, beneID, CLM_ID; 
''')
...
spend_county.registerTempTable("spend_fips")
spend_state = spark.sql('''
select beneID, state, sum(ffs_amount) as ffs_amount_other, sum(mc_amount) as mc_amount_other
FROM spend_fips 
GROUP BY beneID, state; 
''')
...
spend_county.registerTempTable("spend_fips")
spend_state = spark.sql('''
select beneID, state, sum(ffs_amount) as ffs_amount_ed, sum(mc_amount) as mc_amount_ed
FROM spend_fips 
GROUP BY beneID, state; 
''')
...
patient_list = patient_list.filter(col("state").isin(["AK","FL","ME","MT","NM","WY"]))
print(patient_list.count())
patient_list.show()
...
print(patient_list.count())
combined = patient_list.join(ed_spend, on=["beneID","state"], how="left").fillna(0)
combined = combined.join(other_spend, on=["beneID","state"], how="left").fillna(0)
combined = combined.join(ip_spend, on=["beneID","state"], how="left").fillna(0)
combined = combined.select("beneID", "state", "fips_code", "inpatient_cost","total_cost_ed","total_cost_other")
