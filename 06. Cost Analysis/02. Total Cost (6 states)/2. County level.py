...
spend_county.registerTempTable("spend_fips")
spend_state = spark.sql('''
select fips_code, sum(ffs_amount) as ffs_amount_ed, sum(mc_amount) as mc_amount_ed
FROM spend_fips 
GROUP BY fips_code; 
''')
...
combined = combined.select("fips_code","inpatient_cost","total_cost_ed","total_cost_other")
combined.show()

# COMMAND ----------

percent = spark.table("dua_058828_spa240.paper2_acute_care_percent_county")
percent = percent.select("fips_code","state_name","member_month","ip_percent","ed_percent","acute_percent")
...
df_select = combined.select("fips_code","state","avoid_ed_cost","avoid_ip_cost","avoid_acute_cost","pmpy")
df_select = df_select.withColumnRenamed("non_emergent_acute", "avoid_acute")
df_select = df_select.withColumnRenamed("pmpy", "acute_pmpy")
df_select.show()

# COMMAND ----------

df_select.write.format('csv').option('header', 'true').mode('overwrite').save("dbfs:/mnt/dua/dua_058828/SPA240/files/paper2_total_spend.csv")

# COMMAND ----------

dbutils.fs.cp(f"dbfs:/mnt/dua/dua_058828/SPA240/files/paper2_total_spend.csv", "s3://apcws301-transfer/dua/dua_058828/toVDI/paper2_total_spend.csv", recurse=True)
