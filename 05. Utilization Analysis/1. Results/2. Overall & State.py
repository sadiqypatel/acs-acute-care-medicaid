...
columns_to_calculate = total.columns[2:]
...
test = total.select("member_month", *calculate_and_round_ratio(df))

# Show the updated DataFrame
test.show(n=test.count(), truncate=False)
...
df.registerTempTable("total")
total = spark.sql('''
select state_name, sum(total_members) as total_members, sum(member_month) as member_month, sum(non_avoid_ip) as non_avoid_ip, sum(avoid_ip) as avoid_ip, sum(total_ip) as total_ip, sum(non_avoid_ed) as non_avoid_ed, sum(avoid_ed) as avoid_ed, sum(total_ed) as total_ed, sum(non_emergent_acute) as non_emergent_acute, sum(emergent_acute) as emergent_acute, sum(all_cause_acute) as total_acute
FROM total
GROUP BY state_name;
''')
...
columns_to_calculate = total.columns[3:]
...
test.write.saveAsTable("dua_058828_spa240.paper2_acute_care_percent", mode='overwrite')
